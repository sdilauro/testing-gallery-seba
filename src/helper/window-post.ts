...
const defaultPostOption: PostOption = {
  ...
  title: 'Untitled',
  billboard: BillboardMode.BM_NONE
}
...
export function createPost(position: Vector3, options: Partial<PostOption> = {}) {
  const { windowSize, thickness, backgroundColor, fontColor, fontSize, title, billboard } = { ...defaultPostOption, ...options }

  // Root
  const parent = engine.addEntity()
  Transform.create(parent, { position })
  Billboard.create(parent, {billboardMode:billboard})
